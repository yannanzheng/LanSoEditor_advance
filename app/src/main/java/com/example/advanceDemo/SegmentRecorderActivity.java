...
private long lastSegmentsTotalTime;  //这段之前的所有时间之和,单位.  //总时间等于 lastSegmentsTotalTime+ currentSegDuration
@Override
public void onCreate(Bundle savedInstanceState) {
	...
	//此方法不建议用在 分段录制的过程中, 切换前后摄像头的情况.
	//SegmentRecorderActivity设置为竖屏录制,故这里的分辨率是高度大于宽度的.如果是横屏录制,则应把宽高调过来.
//		segmentRecorder = new OSegmentsRecorder(this, cameraTextureView.getHolder(),544,960,2500*1024);//如果是全屏,则建议用这个(需要您修改UI为全屏模式)
//		segmentRecorder = new OSegmentsRecorder(this, cameraTextureView.getHolder(),480,480,1200*1024); //正方形
	segmentRecorder = new OSegmentsRecorder(this, cameraTextureView.getHolder(),480,640,1500*1024);  //推荐采用这个.
//		segmentRecorder = new OSegmentsRecorder(this, cameraTextureView.getHolder(),720,1280,2800*1024);//不建议使用.
//		segmentRecorder = new OSegmentsRecorder(this, cameraTextureView.getHolder(),576,1024,2200*1024);//不建议使用.
	...
	segmentRecorder.setSegmentsRecordListener(new OpenSegmentsRecordListener() {
		...
		public void segmentRecordPause(int timeMS, int segmentIdx) {
			...
			int curSegment=timeMS-(int)lastSegmentsTotalTime;
			
			lastSegmentsTotalTime=timeMS;
			progressView.putTimeList(curSegment);
		}
		...
		public void segmentProgress(long totalTime) {
			...
			long progress=totalTime-lastSegmentsTotalTime;
			progressView.setProgressTime(progress);
		}
		
		@Override  //相机准备好, 返回的预览大小, 你需要根据这个预览大小,来调整画面的实际显示比例.
		public void segmentCameraReady(int[] previewSize) {
			// TODO Auto-generated method stub
			Log.i(TAG,"Semgent Camera size:"+previewSize[0]+"x"+previewSize[1]);
...
private void initView()
{
	...
	progressView.setMinRecordTime(MIN_RECORD_TIME);
	progressView.setMaxRecordTime(MAX_RECORD_TIME);
			...
			if(segmentRecorder!=null)
			{
				...
				if (con) {// 对焦
					if (mAllowTouchFocus) {
						...
						handler.sendEmptyMessageDelayed(MSG_FOCUS_FINISH, 200);
					}
				}
			}
			break;
	...
	switch (v.getId()) {
		case R.id.recorder_cancel:
			...
			if (isDeleteState) {
				...
				lastSegmentsTotalTime-=progressView.getLastTime();
				if(lastSegmentsTotalTime<=0){
